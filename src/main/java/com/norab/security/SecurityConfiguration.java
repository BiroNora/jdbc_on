...
public class SecurityConfiguration {
    ...
    public SecurityFilterChain filterChain(HttpSecurity http) throws Exception {
        http
            ...
            .antMatchers("/swagger-ui/**").hasAnyRole(STAFF.name(), HR.name(), USER.name())
            .antMatchers("/v3/**").hasAnyRole(STAFF.name(), HR.name(), USER.name())
            .antMatchers(HttpMethod.POST, "/api/v1/articles/**").hasRole(USER.name())
            .antMatchers(HttpMethod.PUT, "/api/v1/articles/**").hasRole(USER.name())
